...
public class FXMLAdminAlumnoController implements Initializable, INotificacion {
    ...
    private void btnClicEliminar(ActionEvent event) {
        ...
        if (posicion >= 0){
            ...
            String nombre = String.format("¿Estas seguro de eliminar el registro del alumno(a) %s %s?"
                    + "\nUna vez eliminado no podra ser recuperado",
                    alumno.getNombre(),alumno.getApellidoPaterno());
            if(Utilidad.mostrarAlertaConfirmacion("Eliminar alumno(a)", nombre)){    
                eliminarAlumno(alumno.getIdAlumno());
            }
        }else {
            Utilidad.mostrarAlertaSimple(Alert.AlertType.WARNING, "Selecciona un alumno", "Para eliminar un alumno debes seleccionarlo primero.");
        }
    }
    ...
    private void eliminarAlumno(int idAlumno){
        try {
            ...
            if (!resultado.isError()){
                Utilidad.mostrarAlertaSimple(Alert.AlertType.INFORMATION, "Alumno(a) eliminado",
                        "El registro del alumno fue eliminado correctamente");
                cargarInformacionTabla();
            }else {
                Utilidad.mostrarAlertaSimple(Alert.AlertType.INFORMATION, "Problemas al eliminar",
                        resultado.getMensaje());
            }
        } catch (SQLException ex) {
              Utilidad.mostrarAlertaSimple(Alert.AlertType.INFORMATION, "Problemas al eliminar",
                        "Lo sentimos, ´por el momento no se puede realizar la accion");
        }
    }
